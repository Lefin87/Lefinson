# Дана пицца 8 кусочков. Доступны команды:
# 1. print(‘Взять кусок’)
# 2. print(‘съесть кусок’)
# написать программу, симулующую поедание кусочков пиццы
# (По желанию можно анимацию убывание кусочков пиццы добавить)

pizza = 8
while pizza > 0: #Цикл повторяется пока пицца больше 0
     pizza -= 1
     print(f"Взять кусок ")
     print(f"Съесть кусок ")
else:
    print("Пицца закончилась") #Когда цикл выполнилися без ошибок выводим  пицца закончилась


print("------------------------------------------------")

# Выведите столбец чисел от 1 до 100, используя цикл while

i = 1
while i <= 100:  # Цикл до 100
    print(i)
    i += 1  #Прибавляем еденицу

#Дано число n. Вывести степени этого числа с 1 по 10
#пример

n = 2
for i in range(1,11): #Выбираем диапозон 1-10, 11 заканчивается
    print(f"2^{i} = {n ** i}")
else:
    print("Все готово)")

# Дана пицца 16 кусочков. Доступны команды:
# 1.print(‘Взять кусок’)
# 2. print(‘съесть кусок’)
# 3. print(‘положить кусок обратно’)
# написать программу, симулующую поедание кусочков пиццы, в  программе обязательно должно присутствовать команда №3 два раза. Проверять условием, если пиццы в руках нет.
# решение 1 
for pizza in range(1,17): #Сколько кусков пиццы
     print(f"Взять кусок ")
     if pizza == 3 or pizza == 9: #Эти куски мы ложим
         print(f"положить кусок кусок ")
     else:
         pizza -= 1 #тут мы сьедаем куски
         print(f"Съесть кусок ")
          
# Тут задача если смотреть не глабально а на стол думаю тут правильнее
for pizza in range(1,17): #Сколько кусков пиццы
     print(f"Взять кусок {pizza}")
     if pizza == 3 or pizza == 9: #Эти куски мы ложим
         print(f"положить кусок кусок {pizza}")
         pizza += 1
     else:
        #тут мы сьедаем куски
         print(f"Съесть кусок {pizza} ")
         pizza -= 1
else:
    print(f"Пицца сьедена")
# Все перепроверил логически ничего не правильно, через while тоже не получается 
